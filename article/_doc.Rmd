...
```{r}
#| label: fig-id.5hbpkorf6wua
#| fig-cap: "*Location map with 1304 sampling points in Morropón Province.*"
...
```{r}
#| label: fig-id.xlmsyw2rhth8
#| fig-cap: "*Pearson bivariate correlation matrix between 13 soil physicochemical variables and NDVI. Only statistically significant correlations are shown (p < 0.001).*"

knitr::include_graphics("article/img_1.png")
```
...
```{r}
#| label: fig-id.kz2b6nh44co8
#| fig-cap: "*Relative weights (Wi) of the minimum data set (MDS) indicators used in the estimation of the weighted Soil Quality Index (SQIw).*"
...
```{r}
#| label: fig-id.przdrxehkbhv
#| fig-cap: "*Distribution of the weighted Soil Quality Index (SQIw) in fruits‐growing soils from the Morropon province.*"
...
```{r}
#| label: fig-id.axo2g4578pjz
#| fig-cap: "*Comparison of SQIw across different saline and sodic soil types (a) and climate types (b) in the province of Morropón, based on Dunn’s post hoc test applied after the Kruskal–Wallis test, with Bonferroni correction for multiple comparisons. Statistical significance indicates differences at adjusted p < 0.0001.*"
...
```{r}
#| label: fig-id.iu8chgajkrtn
#| fig-cap: "*Spatial variability map of soil quality (SQIw) in Morropón Province, Piura.*"
