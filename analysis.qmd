...
# 18. Metricas adicionales

```{r}
rf_metrics_by_fold <- cv_df %>%
  group_by(Resample) %>%   # cada fold
  summarise(
...
```{r}
# Errores RK solo donde existe predicción válida
rk_errors <- obs_vals[valid_idx] - rk_pred_points[valid_idx]

rk_summary <- data.frame(
  RMSE_mean = sqrt(mean(rk_errors^2)),
  RMSE_sd   = sd(abs(rk_errors)),
  MAE_mean  = mean(abs(rk_errors)),
  MAE_sd    = sd(abs(rk_errors)),
  R2_mean   = cor(obs_vals[valid_idx], rk_pred_points[valid_idx])^2,
  R2_sd     = sd(
    (obs_vals[valid_idx] - mean(obs_vals[valid_idx])) *
    (rk_pred_points[valid_idx] - mean(rk_pred_points[valid_idx]))
  )
)
