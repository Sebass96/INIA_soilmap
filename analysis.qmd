...
points_geojson <- "gis/JSON/soil_samples.json"
response_field <- "pH"
out_folder <- "ph_RK/"
...
set.seed(123)
k_folds <- 5
rfe_sizes <- c(4, 6, 8, 10) # tamaños candidatos
train_frac <- 0.80 # 80% entrenamiento / 20% prueba

# Modelos candidatos para variograma (autoselección)
...
if (length(tif_files) == 0) stop("No encontré archivos .tif en la carpeta indicada.")

expected_files <- file.path(rasters_folder, paste0(predictor_names, ".tif"))
missing_files <- expected_files[!file.exists(expected_files)]
if (length(missing_files) > 0) {
  stop(
    "Faltan archivos TIFF esperados. Archivos que no encontré:\n",
...
if (is.na(rast_crs) || nchar(rast_crs) == 0) {
  warning("El raster no tiene CRS definido. Se asignará EPSG:32717 (UTM 17S).")
...
if (nrow(pts_df) < 10) warning("Pocos puntos después de limpieza — procede con precaución.")
```
...
cat(
  "Número de puntos - total:", nrow(pts_df),
...
cat("Métricas test guardadas en:", file.path(out_folder, "test_metrics.csv"), "\n")
...
if (inherits(trend_raster, "try-error")) {
  warning("terra::predict con randomForest falló — usando raster::predict como alternativa.")
...
cat("✅ Raster de tendencia (Random Forest final) guardado en:\n", trend_outfile, "\n")
```
...
# Seleccionar el mejor (mínimo SSE válido)
valid_sse <- sse_values[!is.na(sse_values)]
if (length(valid_sse) == 0) {
  warning("❌ Ningún modelo de variograma logró ajuste — se usará el modelo esférico por defecto.")
  best_model_name <- "Sph"
  ...
  message(paste0(
    "✅ Mejor modelo de variograma seleccionado: ", best_model_name,
...
# Guardar variograma empírico
vgm_emp_df <- as.data.frame(vgm_emp)
readr::write_csv(vgm_emp_df, file.path(out_folder, "variograma_empirico.csv"))
cat("Variograma empírico guardado en:", file.path(out_folder, "variograma_empirico.csv"), "\n")

# --------- Extracción de parámetros del modelo seleccionado ---------------
...
cat("📌 Parámetros del variograma guardados en:", file.path(out_folder, "variograma_parametros.csv"), "\n")

# Gráfico PNG variograma empírico + ajuste
vg_png <- file.path(out_folder, "variograma_empirico_ajustado.png")
png(filename = vg_png, width = 900, height = 700)
try(
  {
    plot(vgm_emp, vgm_fit,
      main = paste("Variograma empírico y ajuste (", model_name, ")"),
...
dev.off()
cat("📍 Gráfico del variograma guardado en:", vg_png, "\n")
...
if (inherits(cv_krig, "try-error")) {
  warning("krige.cv falló. No se pudieron calcular métricas LOO de kriging.")
  cv_krig <- NULL
}

# 13.4 Calcular métricas (MAE, RMSE, R2, PBIAS) entre resid_obs y resid_cv_pred
...
if (!is.null(cv_krig)) {
  ...
  # Exportar resultados krige.cv (detallado) y métricas
  ...
  cat("Resultados krige.cv y métricas guardadas en:", out_folder, "\n")
} else {
  cat("No se generaron métricas de kriging (cv_krig es NULL).\n")
}
```
...
```{r}
# 17.1 Métricas CV (del entrenamiento) - RF (CV interno)
...
# 17.2 Preparar la fila Kriging (si se calculó)
...
# Renombrar columnas a estilo 'opción 2' (guion bajo)
...
# 17.6 Gráficos Observed vs Pred - Test y CV
...
# (Opcional) Mostrar mapas interactivos si estás en entorno con GUI
